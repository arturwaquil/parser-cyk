...
void Gramatica::simbInut()
{
    ...
    vector<string> V2, T2;
    V2.push_back(varInicial);  // V2 eh inicializado com a variavel inicial
...
void Gramatica::fncEtapa2()
{
	...
	vector<string> jaIncluidos;
	map <string, string> varsUnit;
    ...
    for (Producao& p: producoes)
    {
        if (p.derivacao.size() >= 2)
        {
            for (string& term: p.derivacao)
            {
                ...
                {
					if (count(jaIncluidos.begin(), jaIncluidos.end(), term))
						term = varsUnit[term];		// se ja existe, usa a variavel Cx com producao Cx > term
					else
					{
	                    Producao temp;
	                    temp.variavel = "C" + to_string(indice);        // cria uma variavel intermediaria cuja unica producao e unitaria e gera esse terminal
	                    temp.derivacao.push_back(term);
	                    P1.push_back(temp);

						variaveis.push_back(temp.variavel);		// adiciona a variavel na gramatica
						jaIncluidos.push_back(term);
						varsUnit[term] = temp.variavel;
...
void Gramatica::fncEtapa3()
{
    ...
    for (Producao& p: producoes)
    {
		...
		{
			Producao temp;
			temp.derivacao.push_back(p.derivacao[0]);
			temp.derivacao.push_back(p.derivacao[1]);
            if (!pertenceDerivacao(temp, P1))
			{
			...
			{
				temp.variavel = retornaVar(temp, P1).variavel;
				p.derivacao.erase(p.derivacao.begin(), p.derivacao.begin()+2);
				p.derivacao.insert(p.derivacao.begin(),temp.variavel);
			}
		}
...
void Gramatica::reconheceCYK(vector<string>& w)
{
	...
	if (count(tabela[n-1][0].begin(), tabela[n-1][0].end(), varInicial))
	{
        ...
        cout << endl << endl << "==================== ARVORES DE DERIVACAO: ====================" << endl << endl;
